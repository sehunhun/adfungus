...
ALTER TABLE ad_crawl_runs ADD COLUMN IF NOT EXISTS job_id TEXT;
...
CREATE INDEX IF NOT EXISTS idx_ad_crawl_runs_job_id ON ad_crawl_runs(job_id, started_at DESC);
...
CREATE TABLE IF NOT EXISTS meta_ads (
    ...
    influencer_anchor_count INTEGER NOT NULL DEFAULT 0,
...
ALTER TABLE meta_ads ADD COLUMN IF NOT EXISTS influencer_anchor_count INTEGER NOT NULL DEFAULT 0;
ALTER TABLE meta_ads ADD COLUMN IF NOT EXISTS is_organic BOOLEAN NOT NULL DEFAULT false;
...
CREATE INDEX IF NOT EXISTS idx_meta_ads_is_organic ON meta_ads(is_organic, first_seen_at ASC);
...
CREATE TABLE IF NOT EXISTS workspace_hidden_brands (
    id BIGSERIAL PRIMARY KEY,
    workspace_id BIGINT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
    brand TEXT NOT NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (workspace_id, brand_normalized)
);

CREATE INDEX IF NOT EXISTS idx_workspace_hidden_brands_workspace
ON workspace_hidden_brands(workspace_id, brand_normalized);
...
def _create_run(
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO ad_crawl_runs (job_id, workspace_id, competitor_id, page_id, library_url)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING id
            """,
            (job_id, workspace_id, competitor_id, page_id, library_url),
        )
...
def _existing_influencer_instagram_usernames(
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT library_id, COALESCE(influencer_instagram_username, '')
            FROM meta_ads
            WHERE library_id = ANY(%s)
            """,
            (library_ids,),
        )
        return {str(row[0]): str(row[1] or "").strip() for row in cur.fetchall()}
...
def _fallback_target_ads(
    new_ads: List[Dict[str, Any]],
    existing_ads: List[Dict[str, Any]],
    existing_usernames: Dict[str, str],
) -> List[Dict[str, Any]]:
    targets = list(new_ads)
    for ad in existing_ads:
        library_id = _library_id(ad)
        if (
            library_id
            and not existing_usernames.get(library_id)
            and bool(ad.get("hasInfluencerContext"))
            and not str(ad.get("influencerInstagramUsername") or "").strip()
            and str(ad.get("influencerDisplayName") or "").strip()
        ):
            targets.append(ad)
    return targets
    ...
    with conn.cursor() as cur:
        for ad in ads:
            ...
            cur.execute(
                ...
                (
                    run_id,
                    str(ad.get("influencerInstagramUsername") or "").strip(),
                    int(ad.get("influencerAnchorCount") or 0),
        ...
        for ad in ads:
            ...
            cur.execute(
                ...
                INSERT INTO meta_ads (
                    ...
                    influencer_anchor_count, is_organic,
                ...
                VALUES (
                    ...
                    %(influencer_anchor_count)s, %(is_organic)s,
                ...
                ON CONFLICT (library_id) DO UPDATE SET
                    ...
                    influencer_anchor_count = EXCLUDED.influencer_anchor_count,
                ...
                """,
                {
                    ...
                    "influencer_anchor_count": int(ad.get("influencerAnchorCount") or 0),
                    "is_organic": bool(ad.get("hasInfluencerContext")),
...
def _process_instagram_metrics_after_crawl(
    ...
    LOGGER.info(
        "instagram metrics starting run_id=%s observed_library_ids=%s",
        run_id,
        len(library_ids),
    )
    ...
    with psycopg.connect(database_url, autocommit=False) as conn:
        ...
        if not competitors:
            ...
            try:
                ...
                if fallback_matches:
                    LOGGER.info(
                        "influencer fallback filled %s ads targets=%s",
                        len(fallback_matches),
                        len(fallback_ads),
                    )
                prepared_new_ads = _prepare_media_for_storage(new_ads)
